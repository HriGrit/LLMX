...
export { MemoryScheduler } from "./scheduler.js";
...
export type OutputSchema<T = unknown> = StandardSchema<T> | Validator<T> | JsonSchema;
...
export type StreamEvent<T = unknown> = ProviderEvent | {
    type: "result";
    result: GenerateResult<T>;
};
export interface StreamResult<T = unknown> extends AsyncIterable<StreamEvent<T>> {
...
export interface ProviderAdapter {
    ...
    parseEmbedding?(body: unknown): {
        embeddings: number[][];
        usage: Usage;
    };
    parseStream?(body: ReadableStream<Uint8Array>, signal: AbortSignal): AsyncIterable<ProviderEvent>;
}
...
export interface ModelAlias {
    ...
    model: string;
    fallback?: string[];
    defaults?: Partial<Pick<GenerateRequest, "temperature" | "topP" | "maxOutputTokens" | "stop">>;
}
...
export interface Exporter {
    export(events: readonly RuntimeEvent[], signal?: AbortSignal): void | Promise<void>;
...
export declare class LLMValidationError extends LLMError {
    constructor(message?: string);
}
export declare class LLMUnsupportedFeatureError extends LLMError {
    constructor(feature: string);
}
export declare function cancelled(): LLMError;
export declare function timeout(phase: string): LLMError;
export declare function httpError(response: Response): LLMError;
export declare function normalizeError(error: unknown, signal?: AbortSignal): LLMError;
```

## gateway

```ts
import type { GatewayConfig, GenerateRequest, GenerateResult, EmbedRequest, EmbedResult, StreamResult } from "./types.js";
export declare function createGateway(input: GatewayConfig): {
...
export declare class MemoryScheduler {
    ...
    constructor(limits?: Limits, clock?: Clock, scopes?: Record<string, Limits>);
    snapshot(): {
        active: number;
        queued: number;
        limits: {
    ...
    acquire(tokens: number, signal?: AbortSignal, scopeKeys?: string[]): Promise<(actual?: number) => void>;
    private pump;
    close(): void;
}
```
...
```ts
import type { ProviderAdapter, ProviderResult, ProviderEvent } from "./types.js";
...
export declare function mockProvider(script?: MockOptions[]): ProviderAdapter;
```
